...
const RECETA: Dictionary = {
	...
	"textura_resultado": "res://sprites/lingote_hierro.png",
	"ingredientes": {
		"mena_hierro": 2
	}
}
...
func _ready() -> void:
	...
	_reiniciar_progreso()
	print("[Horno] Listo. Receta: ", RECETA["resultado"])
...
func _al_entrar(body: Node2D) -> void:
	...
	print("[Horno] Jugador entró. ya_crafteado=", ya_crafteado)
	_mostrar_ui()
...
func _al_salir(body: Node2D) -> void:
	...
	jugador_ref            = null
	label_progreso.visible = false
	print("[Horno] Jugador salió.")

# ── intentar_craftear ─────────────────────────────────────────────────────────
func intentar_craftear() -> void:
	print("[Horno] intentar_craftear() llamado.")

	if ya_crafteado:
		print("[Horno] Ya crafteado, bloqueado.")
		return

	if jugador_ref == null:
		print("[Horno] Error: jugador_ref es null.")
		return

	if jugador_ref.item_en_mano.is_empty():
		print("[Horno] El jugador no tiene item en mano.")
		_flash_label("Coge un ingrediente primero")
		return

	var nombre: String = jugador_ref.item_en_mano["nombre"]
	print("[Horno] Jugador intenta dar: ", nombre)

	if not RECETA["ingredientes"].has(nombre):
		_flash_label("¡" + nombre + " no se necesita!")
		print("[Horno] ", nombre, " no es ingrediente.")
		return
	...
	if ya_dado >= necesario:
		...
		print("[Horno] Ya se completó ese ingrediente.")
		return
	...
	print("[Horno] Absorbido: ", nombre, " → ", nueva_cantidad, "/", necesario)

	jugador_ref.limpiar_mano()
	_mostrar_ui()

	if _receta_completa():
		print("[Horno] ¡Receta completa! Crafteando...")
		_craftear()
...
func _craftear() -> void:
	...
	print("[Horno] Drop creado: ", RECETA["resultado"])
